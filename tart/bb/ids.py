...
from ctypes import (POINTER, c_int, c_uint, c_uint8, c_uint16, c_uint32, c_int32, c_char_p, c_void_p, Structure, c_bool,
                    CFUNCTYPE, py_object)
...
ids_get_token = _func(c_int, POINTER(ids_provider_t), c_char_p, c_char_p, get_token_cb_func, failure_cb_func, py_object,
                      POINTER(c_int))
ids_clear_token = _func(c_int, POINTER(ids_provider_t), c_char_p, c_char_p, clear_token_cb_func, failure_cb_func,
                        py_object, POINTER(c_int))

IDS_MAX_PROPERTY_COUNT = 10
IDS_MAX_PROPERTY_NAME_LEN = 32
IDS_MAX_DATA_NAME_LEN = 128

ids_get_properties = _func(c_int, POINTER(ids_provider_t), c_int, c_int, POINTER(c_char_p), get_properties_cb_func,
                           failure_cb_func, py_object, POINTER(c_uint))
ids_get_data = _func(c_int, POINTER(ids_provider_t), c_int, c_int, c_char_p, get_data_cb_func, failure_cb_func,
                     py_object, POINTER(c_uint))
ids_set_data = _func(c_int, POINTER(ids_provider_t), c_int, c_int, POINTER(ids_data_t), success_cb_func,
                     failure_cb_func, py_object, POINTER(c_uint))
ids_create_data = _func(c_int, POINTER(ids_provider_t), c_int, c_int, POINTER(ids_data_t), success_cb_func,
                        failure_cb_func, py_object, POINTER(c_uint))
ids_delete_data = _func(c_int, POINTER(ids_provider_t), c_int, c_int, c_char_p, success_cb_func, failure_cb_func,
                        py_object, POINTER(c_uint))
ids_list_data = _func(c_int, POINTER(ids_provider_t), c_int, c_int, list_cb_func, failure_cb_func, py_object,
                      POINTER(c_uint))
ids_challenge = _func(c_int, POINTER(ids_provider_t), c_int, c_int, challenge_cb_func, failure_cb_func, py_object,
                      POINTER(c_uint))
